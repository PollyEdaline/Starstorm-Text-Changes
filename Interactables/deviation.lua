...
local sprItemPicker2 = spr.Teleporter--Sprite.load("ItemPicker2", "Interactables/Resources/judgement", 2, 46, 39)

obj.ItemPicker2 = Object.base("MapObject", "ItemPicker2")
obj.ItemPicker2.sprite = sprItemPicker2
obj.ItemPicker2.depth = -6


local intActivity = 25
...
local syncItem = net.Packet.new("SSADItem", function(sender, x, y, item, count)
	local picker = obj.ItemPicker2:findNearest(x, y)
	if picker then
...
local syncData
syncData = net.Packet.new("SSADPickerData", function(sender, player, item, count)
	if net.host then
	...
	if isa(item, "Item") then
		...
		playerData.activatedTimer3 = 200
		...
		if not runData.deviationItems then runData.deviationItems = {} end
		runData.deviationItems[item] = (runData.deviationItems[item] or 0) + count
		
		--playerData._jsitem = optionChoice
		local picker = obj.ItemPicker2:findNearest(playerI.x, playerI.y)
		if picker then
			picker:set("active", 1)
			picker.alpha = 4
			
			local tp = obj.Teleporter:findNearest(playerI.x, playerI.y)
			if tp then
				tp:set("locked", 0)
				tp.visible = true
...
obj.ItemPicker2:addCallback("create", function(self)
	...
	local tele = obj.Teleporter:findNearest(self.x, self.y)
	if tele then
		self.sprite = tele.sprite
		self.subimage = tele.subimage
	end
	...
	local blacklist = {
		...
		[it.HopooFeather] = true,
		[it.DeadMansFoot] = true
	...
	for _, item in ipairs(itp.npc:toList()) do
		if not blacklist[item] then
			if not global.itemAchievements[item] or global.itemAchievements[item]:isComplete() then
				--if not runData.deviationItems or not runData.deviationItems[item] then
					table.insert(validItems, item)
				--end
			end
		end
	end
	if #validItems == 0 then validItems = {it.Fork} end -- if for some reason it doesnt have items to roll, get forks :)
	...
	if net.host then
		...
		for i = 1, 4 do
			...
			table.insert(selfData.itemOptions, {item = item, count = math.min(math.ceil(countCalc * Difficulty.getScaling() * 0.24), 10 * math.max(countCalc, 0.5))})
...
obj.ItemPicker2:addCallback("step", function(self)
	local selfAc = self:getAccessor()
	local selfData = self:getData()
			...
			if player:isValid() and player:control("enter") == input.PRESSED and player:get("activity") ~= intActivity and not selfData.ignoreFrame then
				if not net.online or net.localPlayer == player then
					if misc.getGold() >= selfAc.cost then
						...
						_newInteractables[obj.ItemPicker2].activation(self, player)
					else
						sfx.Error:play()
					end
				end
...
obj.ItemPicker2:addCallback("draw", function(self)
	local selfAc = self:getAccessor()
	local selfData = self:getData()
		...
		if obj.P:findRectangle(self.x - 25, self.y - 40, self.x + 25, self.y + 2) and selfAc.myplayer ~= -4 then
			...
			if pp and player:get("activity") ~= intActivity then
				...
				if input.getPlayerGamepad(player) then
					text = "Press ".."'"..keyStr.."'".." to deviate..."..costStr
				else
					text = "Press ".."&y&'"..keyStr.."'&!&".." to deviate..."..costStr
				end
				graphics.alpha(1)
				graphics.printColor(text, self.x - 60, self.y - 57)
			end
		end
...
table.insert(call.onDraw, function()
	for _, player in ipairs(misc.players) do
		...
		local matchingItemPicker = obj.ItemPicker2:findNearest(player.x, player.y)
		...
		if player:get("activity") == intActivity and matchingItemPicker and matchingItemPicker:isValid() then
			...
			if playerData.activatedTimer3 then
				local givenItem = playerData.givingItemPickerItem
				
				local timerRev = 200 - playerData.activatedTimer3
				
				if isLocalPlayer then
					local backAlpha = thres(playerData.activatedTimer3, 0, 10)
					...
					graphics.color(Color.BLACK)
					graphics.circle(player.x, player.y, (40 + math.cos(global.timer * 0.04) * 10) * thres(playerData.activatedTimer3, 80, 100), false)
					...
					--local itemx, itemy = pointInLine(player.x, playeryy, matchingItemPicker.x, ItemPicker2yy, disProgress)
					local scale = thres(playerData.activatedTimer3, 50, 100)
					...
					if playerData._jsitem and isa(playerData._jsitem, "Item") then
						...
						graphics.drawImage{
							...
							y = playerData._jsitempos2.y,
							scale = scale,
							alpha = thres(playerData.activatedTimer3, 0, 5)
						}
					end
			...
			elseif isLocalPlayer then
				...
				if selItem ~= exitButton.item then
					local tooltipString = selItem.pickupText
					...
					graphics.color(Color.PURPLE)
					graphics.print("CHOOSE A DEVIATION...", matchingItemPicker.x, yoffset - 20, graphics.FONT_LARGE, graphics.ALIGN_MIDDLE)
				end
			end
		end
	end
...
table.insert(call.onPlayerStep, function(player)
	...
	if playerAc.activity == intActivity then
		...
		if matchingItemPicker and matchingItemPicker:isValid() then
			...
			if not net.online or player == net.localPlayer then
				...
				elseif not ItemPickerData.ignoreFrame and not playerData.activatedTimer3 then
			...
			if activation then
				...
				if selectedItem == exitButton.item then
					...
					local tp = obj.Teleporter:findNearest(player.x, player.y)
					if tp then
						tp:set("locked", 0)
						tp.visible = true
						matchingItemPicker.visible = false
					end
				end
				if net.online then
					if net.host then
	...
	if playerData.activatedTimer3 then
		if playerData.activatedTimer3 > 0 then
			playerData.activatedTimer3 = playerData.activatedTimer3 - 3
		else
			playerAc.activity = 0
			playerAc.activity_type = 0
			...
			playerData.activatedTimer3 = nil
		end
	end
end)

return obj.ItemPicker2