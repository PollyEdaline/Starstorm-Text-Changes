...
-- Camera Object
misc.camera = camera --[[{x = 0, y = 0, height = 0, width = 0}
...
-- Outlined Print
function outlinedPrint(str, x, y, bkcol, font, halign, valign)
	local col = graphics.getColor()
	graphics.color(bkcol)
	graphics.print(str, x + 1, y, font, halign, valign)
	graphics.print(str, x, y + 1, font, halign, valign)
	graphics.print(str, x - 1, y, font, halign, valign)
	graphics.print(str, x, y - 1, font, halign, valign)
	graphics.color(col)
	graphics.print(str, x, y, font, halign, valign)
end
function outlinedPrint2(str, x, y, bkcol, font, halign, valign)
	local col = graphics.getColor()
	graphics.color(bkcol)
	graphics.print(str, x + 1, y, font, halign, valign)
	graphics.print(str, x + 1, y + 1, font, halign, valign)
	graphics.print(str, x, y + 1, font, halign, valign)
	graphics.print(str, x - 1, y + 1, font, halign, valign)
	graphics.print(str, x - 1, y, font, halign, valign)
	graphics.print(str, x - 1, y - 1, font, halign, valign)
...
-- Ask Create
askHostCreate = net.Packet.new("SSHCreate", function(player, obj, x, y)
	if obj then
		obj:create(x, y)
	end
end)
...
-- Sync Var
syncInstanceVar = net.Packet.new("SSInstanceVar", function(sender, instance, var, val)
	if instance and var and val then
		...
		if net.host then
			syncInstanceVar:sendAsHost(net.EXCLUDE, sender, instance, var, val)
		end
	end
end)

-- Sync Alarm
syncInstanceAlarm = net.Packet.new("SSInstanceAlarm", function(sender, instance, alarm, val)
	if instance and alarm and val then
		local instanceRes = instance:resolve()
		if instanceRes and instanceRes:isValid() then
			instanceRes:setAlarm(alarm, val)
		end
	end
end)
...
-- Sync Run Data
syncRunData = net.Packet.new("SSRunData", function(sender, key, val)
	if key and val then
		runData[key] = val
		if net.host then
			syncRunData:sendAsHost(net.EXCLUDE, sender, key, val)
		end
	end
end)
...
function drawOutOfScreen(object, txt)
	if not onScreen(object) then
		...
		if isa(object, "ActorInstance") then
			for _, elitetype in ipairs(EliteType.findAll()) do
...
-- Draw Rod Lightning
function drawRodLightning(x1, y1, x2, y2, steps, range, color, alpha)
...
-- Draw Rod Lightning
function drawRodLightning2(x1, y1, x2, y2, steps, range, color)
	local dis = distance(x1, y1, x2, y2)
	local useSteps
	if steps then
		useSteps = math.min(math.max(math.ceil(dis / 40), 2), steps)
...
function typeCheckError(fname, argn, aname, expected, got)
	error(string.format("bad argument #%i ('%s') to '%s' (%s expected, got %s)", argn, aname, fname, expected, type(got)), 3)
end
...
local funcs = {}
local syncInst = net.Packet.new("SSInstSpawn", function(player, object, x, y, instid, funcid, other, other2)
	...
	if other2 then 
		if isa(other2, "NetInstance") then
			arg2 = other2:resolve()
		else
			arg2 = other2
		end
...
function createSynced(object, x, y, id, otherv, otherv2)
	if net.host then
		local inst = object:create(x, y):set("sync", 0)
		if id then
			funcs[id](inst, otherv, otherv2)
		end
		if net.online then
			...
			local other2 = otherv2
			if otherv2 and isa(otherv2, "Instance") then
				if otherv2.getNetIdentity then
					other2 = otherv2:getNetIdentity()
				else
					other2 = nil
				end
			end
			syncInst:sendAsHost(net.ALL, nil, object, x, y, instid, id, other, other2)
		end
	end
end
...
-- Buff syncing
local syncBuff
syncBuff = net.Packet.new("SSBuff", function(sender, actor, buffname, duration)
	...
	if inst and inst:isValid() then
		...
		if buff then
			...
			if net.host then
				syncBuff:sendAsHost(net.EXCLUDE, sender, actor, buffname, duration)
			end
		end
	end
end)
function applySyncedBuff(actor, buff, duration, onlyHost)
	...
	elseif not onlyHost then
		syncBuff:sendAsClient(actor:getNetIdentity(), buff:getName(), duration)
		actor:applyBuff(buff, duration)
	end
end
...
table.insert(call.onImpact, function(damager, x, y)
	...
	if damagerData.bulletTrail then
		if not damagerData.t_pierce then
			if damagerData.bulletTrail == 1 then
				...
				trail.origin = damagerData.originPos
				trail.target = target
				...
				local dis = distance(origin.x, origin.y, x, y)
				local amount = dis / damagerData.t_amount --math.ceil(dis % 30)
				for i = 1, amount do
					local ratio = (i * dis) / amount
					local xx, yy = pointInLine(origin.x, origin.y, x, y, ratio)
					damagerData.t_particle:burst("above", xx, yy, 1, damagerData.t_color)
				end
				damagerData.t_collision = true
			end
		end
	...
	if damagerData.bulletTrail then
		if not damagerData.t_collision or damagerData.t_pierce then
			if damagerData.bulletTrail == 1 then
				...
				local amount = distance / damagerData.t_amount --math.ceil(dis % 30)
				for i = 1, amount do
					local ratio = (i * distance) / amount
					local xx, yy = pointInLine(origin.x, origin.y, target.x, target.y, ratio)
					damagerData.t_particle:burst("above", xx, yy, 1, damagerData.t_color)
				end
			end
		end
...
function addBulletTrailLine(damager, color, size, duration, pierce, animate)
	if damager then
		local damagerData = damager:getData()
		damagerData.bulletTrail = 1
...
function addBulletTrailParticle(damager, particle, color, amount, pierce)
	if damager then
		local damagerData = damager:getData()
		damagerData.bulletTrail = 2
		damagerData.t_particle = particle
		damagerData.t_color = color
		damagerData.t_amount = amount
		damagerData.t_pierce = pierce
	end
end
addBulletTrail = addBulletTrailLine
export("addBulletTrailLine")
...
-- Roll ItemPool
function rollItemPool(rareChance, useChance, uncommonChance, commonChance)
	local item = nil
...
function tameSetElite(actor)
	...
	if actor:getAnimation("palette") or actor:getObject() == obj.Wyvern then
...
function makeTamed(actor, parent) -- this function sucks bad
	...
	if providenceObjects[actor:getObject()] then
		friendProvidence(actor)
	elseif actor:getObject() == obj.GiantJelly then
	...
	elseif actor:getObject() == obj.Wyvern then
		local actorAc = actor:getAccessor()
		if actor:getData().head and actor:getData().head:isValid() then
			actor:getData().head:set("team", actorAc.team)
			actor:getData().head:set("persistent", 1)
		end
		if actor:getData().tail and actor:getData().tail:isValid() then
			actor:getData().tail:set("team", actorAc.team)
			actor:getData().tail:set("persistent", 1)
		end
	end
end
...
getVestigeScaling = function(setting)
	if setting == "items" then
		...
		return (1 + (0.5 * (#misc.players - 1))) * (Difficulty.getScaling("hp") * math.max(Difficulty.getScaling() * 0.2, 1))
	end
end
