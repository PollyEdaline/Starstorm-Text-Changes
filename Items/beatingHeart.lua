...
it.BeatingHeart:setLog{
	...
	story = "My frail body was once more dependent. Not on a shield or any kind of armor, but a beating heart; to keep going, to have one last breath and reach the end.\nThis time, I do not know if I made it out alive. For my body was once more dependent.",
	...
	if not stageBlacklist[Stage.getCurrentStage()] then
		if not runData.bhtimer then
			...
			for _, player in ipairs(obj.EfPlayerDead:findAll()) do
				...
				player:set("death_message", "Alive as long as a fragment remains..." )
			end
