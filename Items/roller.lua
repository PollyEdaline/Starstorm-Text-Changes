...
it.Roller:setLog{
	...
	story = "Pretty old school but pretty useful! It's been a while since I last used this so make sure it is all correct.\nIt's not like it can be broken anyways...",
...
objRoller:addCallback("step", function(self)
	...
	if selfData.life > 0 then
		if self:collidesMap(nextPosX, self.y) then
			if selfData.dt == 3 then
				selfData.life = 0
			else
				selfData.dt = selfData.dt+ 1
			end
		end
		local r = 15
...
it.Roller:addCallback("use", function(player, embryo)
