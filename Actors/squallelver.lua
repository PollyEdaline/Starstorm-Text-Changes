...
local sprPortrait = Sprite.load("SquallElverPortrait", path.."Portrait", 1, 119, 119)
local sprLog = Sprite.load("SquallElverLog", path.."Log", 8, 20, 11)
...
obj.SquallElver:addCallback("create", function(self)
	...
	self:getData().awaitingSprites = true
end)
...
obj.SquallElver:addCallback("step", function(self)
	...
	if controller and controller:isValid() then
		...
		if section == 1 or nextSection and nextSection:isValid() then
			...
			if selfData.awaitingSprites then
				selfData.awaitingSprites = nil
				if section == 1 then
					self.sprite = sprHead
...
mlog.SquallElver.story = "Majestic, that's what I would call them; if it wasn't for how overly territorial their behavior is. They will glide in from afar just to make sure you don't get even a mile near their nests.. Or at least that's what I assume they are protecting.\n\nWhat cautivates me the most about the Elvers is how they manage their flight. I suspect their long, cloth-like appendages may be the culprit, a kind of biological advance I had never seen before, yet,  it is highly reminiscent of fauna commonly found underwater."
mlog.SquallElver.statHP = 200
mlog.SquallElver.statDamage = 14
mlog.SquallElver.statSpeed = 1.2
mlog.SquallElver.sprite = sprLog
mlog.SquallElver.portrait = sprPortrait
